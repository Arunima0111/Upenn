...
featuredir = 'H:\Work\git_repository\radiomics_captk_pipeline\Testing\features';
feature_file = fullfile(featuredir, 'DCIS_R01_consolidated_features.csv')
%feature_file = 'Z:\home\user\features_consolidated.xlsx';
whichSheet = 1; %which sheet in feature_file (just put 1 if only 1 sheet)

savedir = 'H:\Work\git_repository\radiomics_captk_pipeline\Testing\features';
savelab = 'feature_label_example';
savetable = 0; % flag to save the processed features as a CSV or not

% feature names you want to remove altogether (copy the labels directly from feature_file)
%featsToRemove = {'T1_E_GLSZM_Bins_20_Radius_2_ZoneSizeNoneUniformityNormalized'}; 
featsToRemove = [];
