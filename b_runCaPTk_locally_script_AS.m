...
listdir = 'H:\Work\git_repository\radiomics_captk_pipeline\Testing\lists';
ids_list = fullfile(listdir,'id_DCIS_R01.list');
img_list = fullfile(listdir,'input_DCIS_R01.list');
mask_list = fullfile(listdir,'mask_DCIS_R01.list');
output_list = fullfile(listdir,'output_DCIS_R01.list');


% define the CaPTk parameter file path (informs different parameters that
% CaPTk uses to extract features)
%param_file = fullfile(listdir,'params_default_v190_rad1.csv');
paramdir = 'H:\Work\git_repository\radiomics_captk_pipeline\Testing';
param_file = fullfile(paramdir,'params_2pix_resampled_kalina.csv');
